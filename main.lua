...
ParryStatusGroup:AddToggle("ParryMaster", {
    Text    = "Auto Parry",
    Default = false,
    Tooltip = "Master toggle to enable the parry engine. It will automatically press the Parry Key when attacks are detected.",
    Callback = function(v)
        DataBus.ParryState.Active = v
    end,
})

ParryStatusGroup:AddLabel("Keybind\nParry Key"):AddKeyPicker("ParryKeyPicker", {
    Default = "F",
    Mode    = "Always",
    Text    = "Parry Key",
    NoUI    = false,
    Callback = function(v) DataBus.Config.ParryKey = v end,
})
...
DataBus.UI.RefreshParryEntries = function()
    ...
    for animId, entry in pairs(DataBus.ActiveBuild.Entries) do
        ...
        local labelId = "ParryLabel_" .. animId
        ...
        -- Live refresh loop for this specific label
        task.spawn(function()
            while task.wait(0.5) do
                if Library.Unloaded then break end
                local currentLr = DataBus.LearningData[animId]
                if currentLr and Options[labelId] then
                    local c = math.floor(currentLr.Confidence * 100)
                    local t = (currentLr.ManualOverride and currentLr.ManualWindowMs or currentLr.WindowMs) / 1000
                    Options[labelId]:SetText(string.format(
                        "%s%s | %.0f%% conf | %.3fs (%s) | %d✓ %d✗ %d seen",
                        currentLr.Locked and "🔒 " or "",
                        entry.AnimName,
                        c,
                        t,
                        currentLr.ManualOverride and "manual" or "auto",
                        currentLr.Successes,
                        currentLr.Failures,
                        currentLr.TotalEncounters
                    ))
                end
            end
        end)
...
ParryOverrideGroup:AddLabel("Controls the 'Disabled' state for every entry in your current build simultaneously.", true)
...
-- ────────────────────────────────────────────────────────────────────────────
local LoggerTopGroup = Tabs.Logger:AddLeftGroupbox("Silence Hub")
LoggerTopGroup:AddLabel("The Animation Logger and Hit History have been moved to the External Viewer Hub for a tighter workflow.", true)

LoggerTopGroup:AddButton({
    Text = "Open Silence Hub",
    Func = function()
        Modules.Viewer.Open()
    end,
})

local ScannerSettings = Tabs.Logger:AddLeftGroupbox("Scanner Settings")

ScannerSettings:AddSlider("LoggerDistanceSlider", {
...
ScannerSettings:AddButton({
    Text      = "Reset Scanner",
    DoubleClick = true,
    Func      = function()
        DataBus.Animations = {}
        if DataBus.UI.RefreshViewerLogger then DataBus.UI.RefreshViewerLogger() end
        Library:Notify({ Title = "Silence", Description = "Scanner results cleared.", Time = 3 })
    end,
})
